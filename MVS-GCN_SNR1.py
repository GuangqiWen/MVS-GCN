...
class model_gnn(nn.Module):
    ...
    def forward(self, A, A1, A2):
        ...
        simiLoss = 0.0 * (simi_loss6 + simi_loss4 + simi_loss3 + simi_loss2 + simi_loss1 + simi_loss5)
        # 0.2 674 0.1 683 0.3 669 0.4 668 0.5 660 0.05 674 0.08 673 0.15 673

        simiLoss = simiLoss + l.sum() * 0.01
    ...
    def SimiLoss(self, F1, F2):
        ...
        ####
        return simi_loss1
    def SimiLoss3(self, F, F1, F2, F3, F4, F5):
        ...
        for ri in range(self.nums):
            ...
            scores4[ri] = tmp_sum4
            scores5[ri] = tmp_sum5
        return scores, scores1, scores2, scores3, scores4, scores5
...
def main():
    ...
    print(result2)
    print('------------------------------------')
    np.save("result约束2_0.npy", np.array(pres[0]))
    np.save("result约束2_1.npy", np.array(pres[1]))
    np.save("result约束2_2.npy", np.array(pres[2]))
    np.save("result约束2_3.npy", np.array(pres[3]))
    np.save("result约束2_4.npy", np.array(pres[4]))
    for i in range(len(train_data_loaders)):
        ...
        print(test_result)
        result.append(test_result)
    # file = open('./result_约束2.txt', 'w')
    # np.save("result约束2.npy", np.array(pres))
